...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const update = useCallback((id: string, qty: number) => setItems(prev => prev.map(i => i.product.id === id ? { ...i, qty: Math.max(1, qty) } : i)), []);
  const clear = useCallback(() => setItems([]), []);
  const subtotal = useMemo(() => items.reduce((s,i) => s + i.product.price * i.qty, 0), [items]);
  const count = useMemo(() => items.reduce((s,i) => s + i.qty, 0), [items]);

  return <CartCtx.Provider value={{ items, open, setOpen, add, remove, update, subtotal, count, clear }}>{children}</CartCtx.Provider>;
}
