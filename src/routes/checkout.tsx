      ...
      <div className="mt-10 flex items-center gap-3 overflow-x-auto pb-2">
        {steps.map((s, i) => (
          <div key={s} className="flex items-center gap-3">
            <button onClick={() => setStep(i)} className={`flex items-center gap-2 rounded-full px-4 py-2 text-xs font-medium uppercase tracking-[0.15em] transition-colors ${i <= step ? "bg-primary text-primary-foreground" : "border border-border text-muted-foreground"}`}>
        ...
        <motion.div key={step} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-border bg-card p-8">
          {step === 0 && (
            <div className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2"><Field label="Nome" /><Field label="Sobrenome" /></div>
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-3"><Field label="Número" /><Field label="Complemento" /><Field label="Bairro" /></div>
              <div className="grid gap-4 md:grid-cols-2"><Field label="Cidade" /><Field label="Estado" /></div>
            </div>
          )}
          {step === 2 && (
            <div className="space-y-4">
              ...
              <div className="grid gap-3 md:grid-cols-3">
                {["Cartão","Pix","Boleto"].map(m => (
                  <button key={m} className="rounded-2xl border border-border bg-background p-4 text-sm hover:border-primary transition-colors">{m}</button>
                ))}
              </div>
              <Field label="Número do cartão" />
              <div className="grid gap-4 md:grid-cols-3"><Field label="Validade" /><Field label="CVV" /><Field label="Parcelas" /></div>
            </div>
          )}
          {step === 3 && (
            <div className="space-y-4">
          ...
          <div className="mt-8 flex justify-between">
            <button disabled={step === 0} onClick={() => setStep(s => s - 1)} className="rounded-2xl border border-border px-5 py-3 text-xs uppercase tracking-[0.15em] disabled:opacity-40">Voltar</button>
            <button onClick={() => setStep(s => Math.min(steps.length - 1, s + 1))} className="rounded-2xl bg-primary px-6 py-3 text-xs uppercase tracking-[0.15em] text-primary-foreground hover:bg-[#333] transition-colors">
          ...
          <div className="mt-5 space-y-4">
            ...
            {items.map(i => (
              <div key={i.product.id + i.size} className="flex gap-3">
                <img src={i.product.images[0]} alt="" className="h-16 w-14 rounded-lg object-cover" />
                <div className="flex-1 text-sm">
                  <p className="font-medium">{i.product.name}</p>
                  <p className="text-xs text-muted-foreground">Tam {i.size} · Qtd {i.qty}</p>
                </div>
...
function Field({ label, placeholder }: { label: string; placeholder?: string }) {
  return (
    <label className="block">
      ...
      <input placeholder={placeholder} className="w-full rounded-2xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-primary transition-colors" />
    </label>
  );
}
