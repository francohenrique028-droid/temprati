...
type Metric = { label: string; value: number | string; icon: typeof Package; iconClass: string; iconBg: string };
...
function DashboardPage() {
  ...
  const metrics: Metric[] = [
    { label: "Produtos", value: loading ? "—" : products, icon: Package, iconClass: "text-slate-500", iconBg: "bg-slate-100" },
        ...
        <div className="mx-auto max-w-[1400px]">
          ...
          <section className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">
            {metrics.map((metric) => {
              ...
              return <div key={metric.label} className="flex h-[82px] items-center justify-between rounded-2xl border border-[#edf0f4] bg-white px-5 shadow-[0_2px_7px_rgba(15,23,42,0.045)]">
