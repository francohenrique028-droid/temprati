...
function RelatoriosPage() {
  ...
  async function load(isRefresh = false) {
    ...
    const [ordersResult, productsResult, profilesResult, rolesResult] = await Promise.all([
    ...
    else if (rolesResult.error) toast.error(rolesResult.error.message);
    else {
      const adminIds = new Set((rolesResult.data ?? []).map((row: AdminRole) => row.user_id));
      setClients(((profilesResult.data ?? []) as Client[]).filter((profile) => !adminIds.has(profile.id)));
    }
    setLoading(false);
    setRefreshing(false);
  }
  ...
  const metrics = useMemo(() => {
    const paidOrders = orders.filter((order) => order.status !== "cancelled" && ["paid", "authorized"].includes(order.payment_status));
    const revenue = paidOrders.reduce((sum, order) => sum + Number(order.total || 0), 0);
    const pending = orders.filter((order) => order.status === "pending").length;
    const averageTicket = paidOrders.length ? revenue / paidOrders.length : 0;
    return { revenue, paid: paidOrders.length, pending, averageTicket };
  }, [orders]);
  ...
  const chart = useMemo(() => {
    ...
    const values = keys.map((key) => orders.filter((order) => order.status !== "cancelled" && ["paid", "authorized"].includes(order.payment_status) && monthKey(order.created_at) === key).reduce((sum, order) => sum + Number(order.total || 0), 0));
  ...
  const completionRate = orders.length ? `${Math.round((orders.filter((o) => o.status === "delivered").length / orders.length) * 100)}%` : "—";
        ...
        <div className="mx-auto max-w-[1400px]">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-[12px] text-[#7890aa]">Acompanhe o desempenho da loja com dados reais do catálogo, clientes e pedidos.</p>
            </div>
          ...
          <section className="mt-10 grid gap-4 md:grid-cols-4">
            <Stat label="FATURAMENTO" value={loading ? "—" : formatPrice(metrics.revenue)} icon={<TrendingUp className="h-[18px] w-[18px] text-emerald-500" />} />
            <Stat label="PEDIDOS PAGOS" value={loading ? "—" : metrics.paid} icon={<CheckCircle2 className="h-[18px] w-[18px] text-emerald-500" />} />
            <Stat label="PEDIDOS PENDENTES" value={loading ? "—" : metrics.pending} icon={<Clock3 className="h-[18px] w-[18px] text-amber-500" />} />
            <Stat label="TICKET MÉDIO" value={loading ? "—" : formatPrice(metrics.averageTicket)} icon={<ShoppingBag className="h-[18px] w-[18px] text-sky-500" />} />
          </section>
          ...
          <section className="mt-6 grid gap-4 lg:grid-cols-[1.65fr_1fr]">
            <div className="rounded-2xl border border-[#dbe2ea] bg-white p-6 shadow-[0_2px_7px_rgba(15,23,42,0.035)]">
              <div className="flex items-start justify-between">
                <div><h2 className="text-[13px] font-extrabold text-[#10233a]">Faturamento por mês</h2><p className="mt-1 text-[10px] text-[#8a9aae]">Últimos 6 meses · pedidos pagos</p></div>
                <BarChart3 className="h-5 w-5 text-[#94a3b8]" />
              </div>
              {chart.max > 0 ? (
                <div className="mt-8 flex h-52 items-end gap-3 sm:gap-5">
                  {chart.values.map((value, index) => {
                    const height = Math.round((value / chart.max) * 100);
                ...
                <div className="mt-8 flex h-52 flex-col items-center justify-center text-center">
                  <BarChart3 className="h-7 w-7 text-[#cbd5e1]" />
                  <p className="mt-3 text-[11px] font-semibold text-[#64748b]">Sem faturamento registrado</p>
                  <p className="mt-1 text-[10px] text-[#9aa8b8]">Os valores aparecerão quando houver pedidos pagos.</p>
                </div>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-[#dbe2ea] bg-white p-6 shadow-[0_2px_7px_rgba(15,23,42,0.035)]">
              ...
              <div className="mt-6 space-y-5">
                <MiniStat label="Pedidos" value={loading ? "—" : orders.length} icon={<ShoppingBag className="h-4 w-4" />} />
                <MiniStat label="Clientes cadastrados" value={loading ? "—" : clients.length} icon={<Users className="h-4 w-4" />} />
                <MiniStat label="Produtos no catálogo" value={loading ? "—" : products.length} icon={<Package className="h-4 w-4" />} />
                <MiniStat label="Estoque total" value={loading ? "—" : products.reduce((sum, product) => sum + Number(product.stock || 0), 0)} icon={<Package className="h-4 w-4" />} />
              </div>
            </div>
          </section>
          ...
          <section className="mt-6 rounded-2xl border border-[#dbe2ea] bg-white p-6 shadow-[0_2px_7px_rgba(15,23,42,0.035)]">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {statusRows.map(([label, value, color]) => <div key={label} className="flex items-center justify-between rounded-xl border border-[#edf0f4] bg-[#fbfcfe] px-4 py-3"><span className="text-[11px] font-semibold text-[#506784]">{label}</span><span className={`text-[18px] font-extrabold ${color}`}>{loading ? "—" : value}</span></div>)}
            </div>
          </section>

          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="rounded-2xl border border-[#dbe2ea] bg-white p-5"><p className="text-[10px] font-semibold uppercase tracking-[0.09em] text-[#61768d]">Taxa de conclusão</p><p className="mt-2 text-2xl font-extrabold text-[#10233a]">{loading ? "—" : completionRate}</p></div>
            <div className="rounded-2xl border border-[#dbe2ea] bg-white p-5"><p className="text-[10px] font-semibold uppercase tracking-[0.09em] text-[#61768d]">Ticket médio</p><p className="mt-2 text-2xl font-extrabold text-[#10233a]">{loading ? "—" : formatPrice(metrics.averageTicket)}</p></div>
            <div className="rounded-2xl border border-[#dbe2ea] bg-white p-5"><p className="text-[10px] font-semibold uppercase tracking-[0.09em] text-[#61768d]">Pedidos não cancelados</p><p className="mt-2 text-2xl font-extrabold text-[#10233a]">{loading ? "—" : orders.filter((o) => o.status !== "cancelled").length}</p></div>
          </div>
        </div>
      </div>
    </AdminShell>
