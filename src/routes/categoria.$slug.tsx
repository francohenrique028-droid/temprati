...
function CategoryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    else if (["lancamentos", "bestsellers"].includes(slug))
      list = list.filter((p) => p.collection === slug);
              ...
              <div className="flex flex-wrap gap-2">
                {allSizes.map((s) => (
                  <button
                    ...
                    className={`min-w-10 rounded-full border px-3 py-1.5 text-xs transition-colors ${sizes.includes(s) ? "border-primary bg-primary text-primary-foreground" : "border-border hover:border-primary"}`}
                  >
                    {s}
                  </button>
                ))}
              ...
              <div className="flex flex-wrap gap-2">
                {allColors.map((c) => (
                  <button
                    key={c}
                    onClick={() =>
                      setColors((v) => (v.includes(c) ? v.filter((x) => x !== c) : [...v, c]))
                    }
                    aria-label={c}
                    className={`h-8 w-8 rounded-full border ${colors.includes(c) ? "ring-2 ring-primary ring-offset-2 ring-offset-background" : "border-border"}`}
                    style={{ background: c }}
                  />
                ))}
              </div>
        ...
        <div>
          <div className="mb-8 flex items-center justify-between gap-3">
            ...
            <select
              ...
              className="ml-auto rounded-full border border-border bg-card px-5 py-2.5 text-xs outline-none focus:border-primary"
            >
