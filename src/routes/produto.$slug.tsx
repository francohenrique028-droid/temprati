...
function ProductPage() {
  ...
  const uniqueImages = product.images.filter((src: string, index: number, list: string[]) => Boolean(src) && list.indexOf(src) === index);
  const gallery: Media[] = [
  ...
  return (
    <div className="container-x py-8">
      <nav className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <Link to="/categoria/$slug" params={{ slug: product.category }} className="hover:text-foreground capitalize">{product.category}</Link>
        <ChevronRight className="h-3 w-3" />
        ...
        <div className="lg:sticky lg:top-32 lg:self-start">
          ...
          <div className="mt-6 flex items-center gap-4">
            <div className="flex items-center rounded-full border border-border">
              <button onClick={() => setQty(q => Math.max(1, q - 1))} className="p-3"><Minus className="h-3.5 w-3.5" /></button>
              <span className="min-w-8 text-center text-sm font-medium">{qty}</span>
              <button onClick={() => setQty(q => q + 1)} className="p-3"><Plus className="h-3.5 w-3.5" /></button>
            </div>
          ...
          <div className="mt-8 grid gap-4 rounded-2xl border border-border bg-card p-5 text-xs text-muted-foreground">
            <div className="flex items-center gap-3"><Truck className="h-4 w-4" /> Frete grátis acima de R$ 499</div>
            <div className="flex items-center gap-3"><RotateCcw className="h-4 w-4" /> Trocas em até 30 dias</div>
            <div className="flex items-center gap-3"><ShieldCheck className="h-4 w-4" /> Compra 100% segura</div>
          </div>
        </div>
      </div>

      <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-20">
        <Tabs defaultValue="desc">
          <TabsList className="w-full justify-start rounded-none border-b border-border bg-transparent p-0">
            <TabsTrigger value="desc" className="rounded-none border-b-2 border-transparent px-6 pb-4 pt-0 text-xs uppercase tracking-[0.2em] data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none">Descrição</TabsTrigger>
            <TabsTrigger value="size" className="rounded-none border-b-2 border-transparent px-6 pb-4 pt-0 text-xs uppercase tracking-[0.2em] data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none">Medidas</TabsTrigger>
            <TabsTrigger value="rev" className="rounded-none border-b-2 border-transparent px-6 pb-4 pt-0 text-xs uppercase tracking-[0.2em] data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none">Avaliações</TabsTrigger>
          </TabsList>
          <TabsContent value="desc" className="max-w-2xl py-8 text-sm leading-relaxed text-foreground/85">{product.description}</TabsContent>
          <TabsContent value="size" className="max-w-2xl py-8">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-border text-left text-xs uppercase tracking-[0.15em] text-muted-foreground"><th className="py-3">Tam</th><th>Busto</th><th>Cintura</th><th>Quadril</th></tr></thead>
              <tbody>
                {[["P","86","68","94"],["M","90","72","98"],["G","94","76","102"],["GG","98","80","106"]].map(r => (
                  <tr key={r[0]} className="border-b border-border"><td className="py-3 font-medium">{r[0]}</td><td>{r[1]}</td><td>{r[2]}</td><td>{r[3]}</td></tr>
                ))}
              </tbody>
            </table>
          </TabsContent>
          <TabsContent value="rev" className="max-w-2xl py-8 space-y-6">
            {[{n:"Camila R.",t:"Vestiu perfeito, tecido lindo."},{n:"João P.",t:"Excelente acabamento."}].map(r => (
              <div key={r.n} className="border-b border-border pb-6">
                <p className="text-xs uppercase tracking-[0.15em] text-muted-foreground">{r.n} · ★★★★★</p>
