...
export function SiteFooter() {
  ...
  const logo = theme.header.logoText?.trim() || "";
    ...
    <footer data-editor-block="footer" className="mt-24" style={{ background: "var(--tp-footer-bg)", color: "var(--tp-footer-text)" }}>
      <div className="container-x py-16">
        <div className="grid gap-12 md:grid-cols-4">
          <div>
            {logoImage ? (
              <img src={logoImage} alt={logo || "Logo da loja"} className="h-10 max-w-[190px] object-contain object-left" />
            ) : logo ? (
              <div className="text-2xl font-semibold lowercase text-primary">{logo}</div>
            ) : null}
            {f.aboutText && <p className="mt-4 max-w-xs text-sm leading-relaxed opacity-70 italic">{f.aboutText}</p>}
            {(f.instagram || f.facebook || f.whatsapp) && (
        ...
        {f.copyright && <div className="mt-14 flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-8 text-xs opacity-60 md:flex-row"><p>{f.copyright}</p></div>}
      </div>
    </footer>
  );
}
